...
export default class BalanceService {
	...
	static async loadAllBalances(force = false){
		...
		console.log('store.state.scatter.keychain.accounts',store.state.scatter.keychain.accounts);
		const accounts = store.state.scatter.keychain.accounts.reduce((acc, account) => {
			// Filtering out permission based accounts
			//if(!acc.find(x => (x.name !== 'address.bank' && x.identifiable() === account.identifiable()) || (x.name === 'address.bank' && x.formatted() === account.formatted()))) acc.push(account);
